...
abstract: > 
  This is an Quarto notebook to create synthetic data using the faux package to examplify the anaylsis and visualiation of micro scenarios. The generated dataset is similar to data sets exported by Qualtrics and its Loop & Merge function. The data has some predefined properties, such as well-defined means and correlations within and between the evaluation dimensions.
keywords:
  - Micro scenarios
  - R
...
The figure illustrates the layout of a typical data set from survey tools, where each row represents the responses from a single participant.
