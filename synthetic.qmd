...
For generating synthetic data, I use the `faux` package, and guidance for this can be found in the [package vignette](https://cran.r-project.org/web/packages/faux/vignettes/rnorm_multi.html).
...
In this section, I generate synthetic data that simulates properties akin to real survey data. Initially, I create a dataset resembling data from the survey tool Qualtrics.
@fig-surveysdatastructure illustrates the structure of a standard dataset from survey tools, where each row represents the responses from a single participant.
...
For the synthetic data, I specify $N$ topics and two evaluation dimensions, defining that they should be inversely correlated with an intercept.
...
Additionally, I specify that the evaluations of the first and second evaluation dimensions are strongly correlated within topics and negatively correlated between topics.
...
Before proceeding, let's perform a check. Why? A symmetric and positive-definite matrix is required to generate random data based on the defined parameters. A correlation matrix is, by definition, symmetric and positive semi-definite.
If we encounter a correlation matrix that is not positive semi-definite, it could be due to numerical precision issues or errors in computation, particularly when dealing with very large matrices or near-linear dependencies between variables. If this occurs, repeat the steps above—it should work out after a few tries.
