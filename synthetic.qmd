...
library(Matrix)     # nearPD() repairs a near-valid correlation matrix
library(tidyverse)
...
# Random block assembly has no guarantee of producing a mathematically valid
# (positive-definite) correlation matrix, so rnorm_multi() can occasionally fail
# with "correlation matrix not positive definite" here. Instead of looping and
# hoping a re-drawn matrix happens to be valid, project it onto the nearest
# valid positive-definite correlation matrix; this is deterministic, always
# succeeds, and only nudges the requested correlations slightly.
cor_matrix <- as.matrix(Matrix::nearPD(cor_matrix, corr = TRUE, keepDiag = TRUE)$mat)
```
...
The `Matrix::nearPD()` repair step above corrects this automatically, so the diagnostics below should always report success; no re-running or looping is needed.

This step is not necessary if you already have real survey data, as it specifically pertains to the creation of synthetic data.
